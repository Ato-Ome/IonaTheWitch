...
---@param age string
---@param class string
function CharacterSet(id, sex, age, class)
    NPC[id] = {
        State = {
            Current = "Idle",
        },
        Attitude = NPC.Classes[class].Default.Attitude,
        Faction = NPC.Classes[class].Default.Faction,
        Sex = sex,
        Age = age,
        Class = class,
    }
    BlzSetUnitSkin(unit, NPC.Classes[class].Skin[sex][age])
    BlzSetUnitName(unit, NPC.Classes[class].Name[sex][age])
    BlzSetUnitMaxHP(unit, NPC.Classes[class].HP[sex][age])
    SetUnitState(unit, UNIT_STATE_LIFE, GetUnitState(unit, UNIT_STATE_MAX_LIFE))
    BlzSetUnitMaxMana(unit, NPC.Classes[class].MP[sex][age])
    SetUnitState(unit, UNIT_STATE_MANA, GetUnitState(unit, UNIT_STATE_MAX_MANA))
    BlzSetUnitAttackCooldown(unit, NPC.Classes[class].AttackRate[sex][age], 0)
    BlzSetUnitArmor(unit, NPC.Classes[class].Defence[sex][age])
    BlzSetUnitBaseDamage(unit, NPC.Classes[class].Damage[sex][age],0)
end
...
function CreateNPC(player, x, y, sex, age, class, name)
    ...
    CharacterSet(id, sex, age, class)
end

---@param player player
---@param x real
---@param y real
function RandomNPC(player, x, y)
    CreateNPC(player, x, y, NPC.Sex[GetRandomInt(1,2)], NPC.Age[GetRandomInt(1,3)], NPC.Class[GetRandomInt(1,NPC.Class.Count)], "Jack")
end
